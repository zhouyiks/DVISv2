...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher, resume=False, using_thr=False):
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                for j in range(3):
                    ...
                    reference = self.reid_embed(output_1)
                ...
                trcQ_reid_embeds = self.reid_embed(self.track_queries)
                new_ins_reid_embeds = self.reid_embed(new_ins_embeds)
                disappear_reid_embeds = self.reid_embed(disappear_embeds)
                all_reference.append(trcQ_reid_embeds)
                reference = torch.cat([trcQ_reid_embeds, new_ins_reid_embeds], dim=0)

                output_1 = torch.cat([self.track_queries, new_ins_embeds], dim=0)
                pos = torch.cat([detQ_pos, trcQ_reid_embeds], dim=0)
                for j in range(3):
                    output_1 = self.transformer_cross_attention_layers_denoise[j](
                        indentify=output_1, tgt=reference,
                        key=torch.cat([single_frame_reid_embeds, disappear_reid_embeds], dim=0),
                        memory=torch.cat([single_frame_embeds_no_norm, disappear_embeds], dim=0),
                        query_pos=pilot_pos, pos=pos,
            ...
            ms_output = torch.stack(ms_output[1:], dim=0)  # (num_layers, q, b, c)
